			...
			jQuery(document).ready(function() {
				...
				<?php $i = 1; for($i = 1; $i<=30; $i++){ ?>
					...
					$( "#item_name<?php echo $i; ?>" ).autocomplete({
				    	source: '<?php echo site_url("ajaxSale/getSaleData/?");?>',
				    	close: function(){
							var name = $("#item_name<?php echo $i;?>").val();
							var billNo = $('#billNo').val();
							$.post("<?php echo site_url("ajaxSale/getSaleItemData"); ?>", {name : name,billNo : billNo}, function(data){		
								var d = jQuery.parseJSON(data);				
								 $('#item_no<?php echo $i; ?>').val(d.itemNo);
								 $('#item_quantity<?php echo $i; ?>').val(d.item_quantity);
								 $('#item_price<?php echo $i; ?>').val(d.prise_per_pro);
								 $('#item_discount<?php echo $i; ?>').val(d.discount);
								 $('#discount<?php echo $i; ?>').val(d.discount_rate);
								 $('#total_price<?php echo $i; ?>').val(d.total);
								 $('#sub_total<?php echo $i; ?>').val(d.sub_total);
							});
						}
				    });
